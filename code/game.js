...
DOMDisplay.prototype.scrollPlayerIntoView = function() {
  ...
  var center = player.pos.plus(player.size.times(2))
                 .times(scale);
...
Spike2.prototype.act = function(step, level){
	var newPos = this.pos.plus(this.speed.times(step));
	if (!level.obstacleAt(newPos, this.size))
		this.pos = newPos;
    else if (this.repeatPos)
...
var wobbleSpeed = 10, eyeWobbleDist = .05;

Eye.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * eyeWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Enemy2.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * eyeWobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null ||
		type == "spike" && this.status == null ||
		type == "spike2" && this.status == null ||
		type == "enemy" && this.status == null ||
		type == "enemy2" && this.status == null ||
		type == "eye" && this.status == null) {
		this.status = "lost";
...
function runGame(plans, Display) {
	var count = 3;
	var astronaut = document.getElementById('astro');
